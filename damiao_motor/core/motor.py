...
class DaMiaoMotor:
    ...
    # Decode feedback
    # -----------------------
    def process_feedback_frame(
        self, data: bytes, arbitration_id: int | None = None
    ) -> None:
    ...
    def decode_register_reply(
        ...
        with self.registers_lock:
            ...
            if REGISTER_TABLE[data[3]].data_type == "float":
                ...
                raise ValueError(
                    f"Unknown data_type: {REGISTER_TABLE[data[3]].data_type} for register {data[3]}"
                )
            return

    def decode_sensor_feedback(
        self, data: bytes, arbitration_id: int | None = None
    ) -> Dict[str, float]:
        if len(data) != 8:
    ...
    def _send_register_cmd(
        self, cmd_byte: int, rid: int, data: Optional[bytes] = None
    ) -> None:
        """
