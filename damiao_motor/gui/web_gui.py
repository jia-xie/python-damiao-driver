...
from damiao_motor.core.motor import REGISTER_TABLE, MOTOR_TYPE_PRESETS
...
def connect():
    ...
    try:
        ...
        if (
            "Error Code 100" in error_msg
            or "Network is down" in error_msg
            or "[Errno 100]" in error_msg
        ):
            hint = (
...
def scan():
    ...
    try:
        if _controller is None:
            return jsonify(
                {"success": False, "error": "Not connected. Please connect first."}
            ), 400
        ...
        try:
            ...
            if (
                ...
                return jsonify(
                    {"success": False, "error": error_msg, "hint": hint}
                ), 500
        ...
        if (
            ...
            channel = (
                getattr(_controller.bus, "channel", "can0") if _controller else "can0"
            )
            hint = (
                f"To fix this, bring up the CAN interface:\n"
                f"  sudo ip link set {channel} up type can bitrate 1000000\n\n"
...
def get_registers(motor_id: int):
    ...
    try:
        if motor_id not in _motors:
            return jsonify(
                {"success": False, "error": f"Motor {motor_id} not found"}
            ), 404
...
def set_register(motor_id: int, rid: int):
    ...
    try:
        ...
        return jsonify(
            {"success": False, "error": f"Register {rid} not found in register table"}
        ), 400
    except ValueError as e:
        return jsonify({"success": False, "error": f"Invalid value: {str(e)}"}), 400
    except TimeoutError as e:
        return jsonify(
            {"success": False, "error": f"Timeout writing register: {str(e)}"}
        ), 500
    except Exception as e:
        import traceback
...
def send_motor_command(motor_id: int):
    ...
    try:
        ...
        pos_vel_velocity_limit = data.get(
            "velocity_limit", data.get("target_velocity", 0.0)
        )
        torque_limit_ratio = data.get("torque_limit_ratio", 0.0)
        ...
        if control_mode == "MIT":
            ...
            return jsonify(
                {"success": False, "error": f"Unknown control_mode: {control_mode}"}
            ), 400
...
def run_server(host="127.0.0.1", port=5000, debug=False, production=False):
    ...
    print("Starting DaMiao Motor Parameter Editor...")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Web-based GUI for DaMiao motor parameters"
    )
    parser.add_argument(
